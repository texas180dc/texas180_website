  ...
  return (
    <>
      ...
      >
        <nav
          aria-label="Main"
          className="relative mx-auto flex h-[68px] w-full max-w-[1120px] items-center justify-between px-6 md:px-10"
        >
          {/* Wordmark */}
          <Link
            href="/"
          ...
          {/*
            Desktop tabs.
            Each link is a padded pill rather than bare text — the whole pill is
            clickable, so the target is ~40px tall instead of the ~16px of a
            text baseline. That matters on a laptop trackpad and is the minimum
          ...
          <div className="absolute left-1/2 hidden -translate-x-1/2 items-center gap-1 md:flex">
            {NAV_LINKS.map((link) => (
              <Link
                ...
                className={`rounded-full px-4 py-2 text-[14px] tracking-wide transition-colors duration-200 ${
                  isActive(link.href)
